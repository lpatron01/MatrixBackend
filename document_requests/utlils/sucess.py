...
class ArabicCertificateGenerator:
    ...
    def __init__(self, font_path='arial.ttf', bold_font_path=None, alanguage='ar'):
        ...
        """
        # Construct paths for fonts using BASE_DIR (same as inscri.py and presence.py)
    ...
    def generate_certificate(self, output_path, alanguage='ar', **params):
        """
        Generate success certificate PDF (FULL A4 PAGE)
        ...
        logger.info(f"  Page size: FULL A4 ({page_width:.1f} x {page_height:.1f} points)")
        logger.info(f"  Seuls les VALEURS des paramètres sont affichés en gras, les étiquettes sont normales (fr) | Only parameter values are in bold, labels are normal (ar)")
        return True
